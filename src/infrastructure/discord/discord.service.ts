...
export class DiscordService implements OnModuleInit {
  ...
  private async handleMessage(message: Message) {
    ...
    try {
      // Comandos de chat usam memória
      if (
        commandName === 'chat' ||
        commandName === 'c' ||
        commandName === 'conversar'
      ) {
        await this.handleChatWithMemory(message, args);
      } else {
  ...
  private async handleChatWithMemory(message: Message, args: string[]) {
    const userId = message.author.id;
    const userName = message.author.username;
    const channelId = message.channel.id;
    ...
    await this.chatCommand.execute(messageWrapper, args);
  }
}
